...
wt = (
    '''                                                
  [[gr]]OPEN SOURCE [[ye]]TELEGRAM CHANNEL MEMBER ADDER [[re]] SCRIPT
  
  github.com/getdzypher
    '''
)
COLORS = {
    "re": "\u001b[31;1m",
...
if os.path.isfile('channeladder_log.txt'):
    with open('channeladder_log.txt', 'r') as r:
        data = r.readlines()
    api_id = data[0]
    api_hash = data[1]
    ...
    with open('channeladder_log.txt', 'w') as a:
        a.write(api_id + '\n' + api_hash)

client = TelegramClient('dzypher', api_id, api_hash)
...
async def main():
    ...
    with open('channeladder_members.txt', 'w') as w:
